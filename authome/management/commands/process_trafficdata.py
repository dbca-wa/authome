...
from authome import utils
from authome.views.monitorviews import  _save_trafficdata,_sum,_add_avg
...
class Command(BaseCommand):
    ...
    def handle(self,*args,**options):
        logger.info("Begin to process traffic data.")
        #get a batchid
        batchid = timezone.localtime()
        ...
        try:
            if settings.AUTH2_CLUSTERID:
                self._save_cluster_traffic_data(batchid)
            else:
                self._save_traffic_data(batchid)
        except:
    ...
    def _save_cluster_traffic_data(self,batchid):
    ...
    def _save_traffic_data(self,batchid):
        try:
    ...
    def _populate_reports(self):
        ...
        for process_status in process_status_qs:
            ...
            for data in itertools.chain(data_qs,[models.TrafficData(batchid=self.BATCHID_END)]):
