    ...
    def test_previous_session_cache(self):
        ...
        for commands in [
            ...
            try:
                ...
                res.raise_for_status()
                self.assertEqual(res.cookies.get(settings.SESSION_COOKIE_NAME),None,msg="Session is migrated from previous session cache. and the client session cookie should not be changed, response should not return session cookie..commands={}".format(commands))
                ...
                self.assertEqual(self.is_session_deleted(session_cookie,"standalone"),True,msg="The migrated session data should be '{{\"migrated\":True}}'.commands={}".format(commands))
    ...
    def test_migrate_to_default_cluster(self):
        ...
        for commands in [
            ...
            try:
                ...
                self.assertEqual(session_cookie,auth01_session_key,msg="Cluster session id should be populated from original session key.commands={}".format(commands))
                ...
                if cache_key_prefix == auth01_cache_key_prefix:
                    self.assertEqual(user_profile3["authenticated"],True,msg="The cluster session has the same cache key as the non-cluster session, the previous session cookie is still accessable.commands={}".format(commands))
                else:
                    self.assertEqual(user_profile3["authenticated"],False,msg="The cluster session has the different cache key from the non-cluster session, the previous session cookie is not available.commands={}".format(commands))
                    self.assertEqual(self.is_session_deleted(session_cookie,"standalone"),True,msg="The cluster session has the different cache key with the non-cluster session, the previous session cookie should be deleted during migration.commands={}".format(commands))
    ...
    def test_migrate_to_default_cluster_with_previous_cache(self):
        ...
        for commands in [
            ...
            try:
                ...
                self.assertEqual(session_cookie,auth02_session_key,msg="Cluster session id should be populated from original session key.commands={}".format(commands))
                ...
                if standalone2_cache_key_prefix == auth02a_cache_key_prefix:
                    self.assertEqual(user_profile4["authenticated"],True,msg="The cluster session has the same cache key as the non-cluster session, the previous session cookie is still accessable.commands={}".format(commands))
                else:
                    self.assertEqual(user_profile4["authenticated"],False,msg="The cluster session has the different cache key with the non-cluster session, the previous session cookie is not available.commands={}".format(commands))
                    self.assertEqual(self.is_session_deleted(session_cookie,"standalone2"),True,msg="The migrated session data should be '{{\"migrated\":True}}'.commands={}".format(commands))
    ...
    def test_migrate_to_other_cluster(self):
        ...
        for commands in [
            ...
            try:
                ...
                user_profile3 = res.json()
                self.assertEqual(user_profile3["authenticated"],False,msg="The original session has been migrated, can't use it to access anymore.commands=".format(commands))
                self.assertEqual(self.is_session_deleted(session_cookie,"standalone"),True,msg="The migrated session should be deleted from original server 'standalone'.commands={}".format(commands))

                self.assertEqual(self.is_session_deleted(session_cookie,"auth01"),True,msg="The migrated session data should be delete from default cluster server 'auth01' .commands={}".format(commands))
    ...
    def test_migrate_to_other_cluster_with_previous_cache(self):
        ...
        for commands in [
            ...
            try:
                ...
                self.assertEqual(session_cookie,auth03_session_key,msg="Cluster session id should be populated from original session key.commands={}".format(commands))
                ...
                self.assertEqual(user_profile5["authenticated"],False,msg="The original session has been deleted, can't use it to access anymore.commands={}".format(commands))

                auth02_session_cookie = self.get_cluster_session_cookie("AUTH2_02",auth03_session_cookie)
                self.assertEqual(self.is_session_deleted(session_cookie,"auth02a"),True,msg="The migrated session data should be deleted.commands={}".format(commands))
    ...
    def test_migrate_session_among_clusters(self):
        ...
        for commands in [
            ...
            try:
                ...
                self.assertEqual(auth02_session_key,auth01_session_key,msg="The session key should not be changed during migration.commands=".format(commands))
                auth02_user_profile = res.json()
                self.assertEqual(auth02_user_profile["authenticated"],True,msg="User should have already loged in.commands=".format(commands))
                auth02_session_data = self.get_session_data(auth02_session_cookie,"auth02")
                ...
                self.assertEqual(self.is_session_deleted(auth01_session_cookie,"auth01"),True,msg="The migrated session data should be '{{\"migrated\":True}}'.commands={}".format(commands))
                ...
                self.assertEqual(self.is_session_deleted(auth02_session_cookie,"auth02"),True,msg="The migrated session data should be deleted.commands={}".format(commands))
    ...
    def test_cluster_with_previous_cache(self):
        ...
        for commands in [
            ...
            try:
                ...
                self.assertEqual(self.is_session_deleted(auth01_session_cookie,"auth01"),True,msg="The migrated session data should be deleted.commands={}".format(commands))
    ...
    def test_migrate_among_clusters_with_previous_cache(self):
        ...
        for commands in [
            ...
            try:
                ...
                self.assertEqual(auth03_session_key,auth01_session_key,msg="The random generated session key(not including the hash value of lb hash key and auth2 cluster id) should be the same value as the migrated session key.commands=".format(commands))
                self.assertEqual(auth03_user_profile["authenticated"],True,msg="User should have already loged in.commands={}".format(commands))
                auth03_session_data = self.get_session_data(auth03_session_cookie,"auth03")
                ...
                self.assertEqual(self.is_session_deleted(auth01_session_cookie,"auth01a"),True,msg="The migrated session data should be '{{\"migrated\":True}}'.commands={}".format(commands))
